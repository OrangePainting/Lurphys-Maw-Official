...
var transparent_ordering_mode: MoldWorldSettings.TransparentOrderingMode:
	...
	set(value):
		_ensure_initialized()
		_active_settings.transparent_ordering_mode = value
...
var statistics: MoldStatistics:
	get:
		...
		var lod_counts := _lod_counts()
		return MoldStatistics.new(_active_count, _active_retained_batch_count() + immediate_batches + _gpu_batch_count(), _mesh_cache.count, _mesh_cache.hits, _mesh_cache.misses, _immediate_command_count, immediate_batches, lod_counts[0], lod_counts[1], lod_counts[2], lod_counts[3], lod_counts[4], lod_counts[5], _last_lod_migration_count)
...
func _can_use_gpu_polyline(index: int) -> bool:
	return _polyline_policies.get(index, MoldPolylineBackend.Mode.CPU) == MoldPolylineBackend.Mode.AUTO and MoldGpuPolylineRenderer.is_supported() and _polylines[index]._points.size() <= MoldGpuPolylineRenderer.MAX_TEXELS/2 and _color_modes[index] in [MoldStyle.ColorMode.SINGLE, MoldStyle.ColorMode.DUAL_GRADIENT] and (_style_modes[index] != MoldStyle.BlendMode.CUSTOM or MoldGpuPolylineRenderer.is_material_compatible(_resolve_custom_material(_style_for(index))))

func _sync_gpu_polylines() -> void:
	for key in _polyline_policies:
		...
		gpu.set_visible(_visible[index] != 0 and _local_scales[index] != Vector3.ZERO)
		gpu.local_aa_quality = _local_aa_quality[index]
	for renderer in _gpu_polyline_renderers: renderer._sync()
...
class ImmediateTemplate:
	...
	func capture(value: MoldShape) -> void:
		...
		shape.dash.fill_count = value.dash.fill_count
		shape.dash.total_count = value.dash.total_count
		...
		shape.dash.length = value.dash.length
		shape.dash.uses_fixed_size = value.dash.uses_fixed_size
...
var _last_lod_migration_count := 0
...
func _validate_requested_settings(requested: MoldWorldSettings) -> void:
	...
	if _active_settings.transparent_ordering_mode != value.transparent_ordering_mode or _active_settings.capacity != value.capacity or _active_settings.roundness_resolution != value.roundness_resolution or _active_settings.retained_unused_mesh_count != value.retained_unused_mesh_count or _active_settings.local_aa_quality != value.local_aa_quality or _active_settings.culling_mode != value.culling_mode or not is_equal_approx(_active_settings.culling_bounds_padding, value.culling_bounds_padding) or _active_settings.lod_mode != value.lod_mode or not is_equal_approx(_active_settings.lod_bias, value.lod_bias) or _active_settings.lod_threshold_pixels != value.lod_threshold_pixels or not is_equal_approx(_active_settings.lod_hysteresis, value.lod_hysteresis) or not is_equal_approx(_active_settings.lod_transition_width, value.lod_transition_width) or _active_settings.lod_evaluation_budget != value.lod_evaluation_budget:
...
func _process(_delta: float) -> void:
	...
	_sync_gpu_polylines()
	_evaluate_retained_lod(get_viewport().get_camera_3d())
	performance_metrics.end_lod(render_start)
	_renderer.sync(_render_batches, _retired_batches)
...
func begin_immediate() -> MoldImmediate:
	...
	return MoldImmediate.new(self, _immediate_generation)


func draw(callback: Callable) -> void:
	var immediate := begin_immediate()
...
func _record_immediate(generation: int, shape: MoldShape, position: Vector3, rotation: Quaternion, scale: Vector3, style: MoldStyle, render_state: MoldRenderState) -> void:
	...
	if generation <= 0 or generation != _immediate_generation:
		push_error("This immediate frame was replaced by a newer begin_immediate call.")
		return
	if not shape: return
	...
	if _immediate_templates.is_empty(): _immediate_templates.resize(MoldShape.Kind.TORUS + 1)
	...
	if shape_match:
		...
		layout = MoldShapeLayout.resolve(shape)
	...
	var has_secondary: bool = selection[3]
	...
	if has_secondary:
		_add_immediate_shape(shape, style, render_state,
			_mesh_cache.resolve_key(shape, selection[1]), transform, custom_data,
...
func _add_immediate_shape(shape: MoldShape, style: MoldStyle, render_state: MoldRenderState, mesh_key, transform: Transform3D, custom_data: Vector4, lod_data: Vector4, template: ImmediateTemplate, dash_data: Vector4, prepared: bool) -> void:
	...
	if not batch:
		...
		batch.configure_immediate(key, mesh_key, _mesh_cache.acquire(mesh_key), style.mode, custom_material, render_state, shape.is_2d, _active_settings.local_aa_quality, coverage_output)
		batches[key] = batch
	...
	if not prepared:
		batch.add_immediate(
			...
			_gradient_data_for(shape, style.color_mode, style.color_interpolation, style.gradient_space, style.gradient_direction), lod_data)
		return
	...
	batch.add_immediate(
		...
		dash_data, template.flags, template.gradient_data, lod_data)
...
func _record_immediate_polyline(generation: int, polyline: MoldPolyline, position: Vector3, rotation: Quaternion, scale: Vector3, style: MoldStyle, render_state: MoldRenderState) -> void:
	...
	if not batch:
		batch = MoldBatch.new()
		_render_batches.append(batch)
		batch.configure_immediate(key, mesh_key, _mesh_cache.acquire(mesh_key), style.mode, custom_material, render_state, false, _active_settings.local_aa_quality, coverage_output)
		batches[key] = batch
...
func _create_polyline_internal(polyline: MoldPolyline, style: MoldStyle, render_state: MoldRenderState, position: Vector3, rotation: Quaternion, scale: Vector3, backend: MoldPolylineBackend.Mode) -> MoldHandle:
	...
	_style_modes[index] = style.mode; _style_materials[index] = style.custom_material; _colors[index] = style.color; _secondary_colors[index] = style.secondary_color
	_emissions[index] = style.emission_strength; _color_modes[index] = style.color_mode
...
func update_style(handle: MoldHandle, style: MoldStyle) -> void:
	...
	var material_changed := _style_modes[index] != style.mode or not _material_handles_equal(_style_materials[index], style.custom_material)
...
func update_dash(handle: MoldHandle, dash: MoldDash) -> void:
	...
	if index < 0: return
	if _polylines[index]:
		push_error("Polyline dashes are not supported.")
		return
...
func update_line_positions(handle: MoldHandle, start: Vector3, end: Vector3) -> void:
	...
	if not shape: return
	_update_line_shape(index, shape)
	_write_properties(index)
	_write_transform(index)
...
func _update_line_shape(index: int, shape: MoldShape) -> void:
...
static func _material_id_for(style: MoldStyle) -> int:
...
func _ensure_initialized() -> void:
	...
	_mesh_cache = MoldMeshCache.new(_active_settings.roundness_resolution, _active_settings.retained_unused_mesh_count)
...
func _bind(index: int) -> void:
	...
	if _visible[index] == 0 or _local_scales[index] == Vector3.ZERO \
			...
			or (_shapes[index] and _shapes[index].is_empty): return
...
func _bind_one(index: int, secondary: bool) -> void:
	...
	var material_id := _style_materials[index]._id if _style_materials[index] else 0
	...
	if not batch:
		...
		batch.configure(key, mesh, _style_modes[index], custom_material, _render_states[index], not _polylines[index] and _shapes[index].is_2d, _local_aa_quality[index], coverage_output)
		batches[key] = batch
...
func _unbind_one(index: int, secondary: bool) -> void:
	...
	# style migrations; the renderer simply hides a zero-count batch.
...
func _angular_lod_data(shape: MoldShape, lod_data: Vector4) -> Vector4:
	if shape and not shape.is_line and shape.kind in [MoldShape.Kind.RECTANGLE,
...
func _evaluate_lod(index: int, camera: Camera3D) -> void:
	...
	if _is_lod_eligible(shape):
		...
		if _active_settings.lod_mode == MoldWorldSettings.LodMode.AUTOMATIC:
...
func _select_immediate_lod(shape: MoldShape, transform: Transform3D, camera: Camera3D, custom_data: Vector4, template: ImmediateTemplate) -> Array:
	var primary: int = shape.detail
	var secondary: int = shape.detail
	var blend := 0.0
	var has_secondary := false
	if _active_settings.lod_mode == MoldWorldSettings.LodMode.MANUAL or not camera or not _is_lod_eligible(shape):
		return [primary, secondary, blend, has_secondary]
...
static func _is_lod_eligible(shape: MoldShape) -> bool:
	return not shape.is_empty and not shape.is_2d and not (shape.kind == MoldShape.Kind.CUBOID and shape.roundness <= 0.0) and not (shape.kind == MoldShape.Kind.REGULAR_PRISM and shape.roundness <= 0.0)
...
static func _lod_world_diameter(transform: Transform3D, shape: MoldShape, custom_data: Vector4) -> float:
	...
	var diameter: float
	match shape.kind:
		MoldShape.Kind.SPHERE, MoldShape.Kind.HEMISPHERE, MoldShape.Kind.TORUS:
...
func _property_flags_for(shape: MoldShape, emission: float, local_aa_quality: MoldWorldSettings.LocalAaQuality) -> Vector4:
	var deformation := 1.0 if shape.kind == MoldShape.Kind.CAPSULE else (2.0 if shape.kind == MoldShape.Kind.TORUS else 0.0)
...
func _gradient_data_for(shape: MoldShape, color_mode: MoldStyle.ColorMode, color_interpolation: MoldStyle.ColorInterpolation, gradient_space: MoldStyle.GradientSpace, gradient_direction: Vector3) -> Vector4:
	...
	if color_mode == MoldStyle.ColorMode.DUAL_RADIAL:
		if shape.kind in [MoldShape.Kind.RECTANGLE, MoldShape.Kind.RECTANGLE_RIM]: direction = Vector3(shape.size.x, shape.size.y, 0.0)
		elif shape.kind in [MoldShape.Kind.REGULAR_POLYGON, MoldShape.Kind.REGULAR_POLYGON_RIM]: direction = Vector3(shape.radius * 2.0, shape.radius * 2.0, 0.0)
	if not shape.is_2d and color_mode in [MoldStyle.ColorMode.DUAL_RADIAL, MoldStyle.ColorMode.DUAL_RADIAL_BOUNDS]:
		direction = MoldShapeLayout.resolve(shape).scale
...
func _lod_counts() -> PackedInt32Array:
	var result := PackedInt32Array([0, 0, 0, 0, 0, 0])
	for index in _alive.size():
		if _alive[index] == 0 or _polylines[index] or _shapes[index].is_empty \
				or _local_scales[index] == Vector3.ZERO: continue
		result[clampi(_resolved_details[index], 0, 4)] += 1
		if _has_secondary_lod[index] != 0: result[5] += 1
	return result


func _style_for(index: int) -> MoldStyle:
	return MoldStyle.new(_style_modes[index], _colors[index], _secondary_colors[index], _emissions[index], _color_modes[index], _color_interpolations[index], _gradient_spaces[index], _gradient_directions[index], _style_materials[index])

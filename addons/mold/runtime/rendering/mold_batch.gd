...
func configure(value_key: Vector4i, value_mesh: Mesh, value_mode: MoldStyle.BlendMode, value_custom_material: ShaderMaterial, state: MoldRenderState, value_is_2d: bool, value_local_aa_quality: MoldWorldSettings.LocalAaQuality, value_coverage_output := false) -> void:
	key = value_key
	mesh = value_mesh
	mode = value_mode
...
func configure_immediate(value_key: Vector4i, value_mesh_key, value_mesh: Mesh, value_mode: MoldStyle.BlendMode, value_custom_material: ShaderMaterial, state: MoldRenderState, value_is_2d: bool, value_local_aa_quality: MoldWorldSettings.LocalAaQuality, value_coverage_output := false) -> void:
	configure(value_key, value_mesh, value_mode, value_custom_material, state, value_is_2d, value_local_aa_quality, value_coverage_output)
	is_immediate = true
...
func add_item(slot: int, transform: Transform3D, primary_color: Color, secondary_color: Color, custom_data: Vector4, dash_data: Vector4, flags: Vector4, gradient_data: Vector4, lod_data := Vector4(1.0, 0.0, 0.0, 0.0), secondary_membership := false) -> int:
...
func add_immediate(bounds: AABB, transform: Transform3D, primary_color: Color, secondary_color: Color, custom_data: Vector4, dash_data: Vector4, flags: Vector4, gradient_data: Vector4, lod_data := Vector4(1.0, 0.0, 0.0, 0.0)) -> int:
...
func write_properties(index: int, primary_color: Color, secondary_color: Color, custom_data: Vector4, dash_data: Vector4, flags: Vector4, gradient_data: Vector4, lod_data := Vector4(1.0, 0.0, 0.0, 0.0)) -> void:
	if index < 0 or index >= count(): return
	_store_properties(index, primary_color, secondary_color, custom_data, dash_data, flags, gradient_data, lod_data)
	if not property_pending:
		property_revision += 1

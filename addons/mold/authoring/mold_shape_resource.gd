...
@export_range(0.0, 100000.0, 0.001, "or_greater") var radius := 0.5: set = _set_radius
@export_range(0.0, 100000.0, 0.001, "or_greater") var height := 1.0: set = _set_height
@export_range(0.0, 100000.0, 0.001, "or_greater") var thickness := 0.1: set = _set_thickness
...
@export_range(-3600.0, 3600.0, 0.1, "or_less", "or_greater") var start_degrees := 0.0: set = _set_start_degrees
@export_range(0.0, 360.0, 0.1) var span_degrees := 360.0: set = _set_span_degrees
...
@export_range(0, 4096, 1) var dash_fill_count := 8: set = _set_dash_fill_count
@export_range(1, 4096, 1) var dash_total_count := 8: set = _set_dash_total_count
@export_range(0.0, 1.0, 0.001) var dash_spacing := 0.5: set = _set_dash_spacing
@export var dash_offset := 0.0: set = _set_dash_offset
...
func _set_dashed(value: bool) -> void:
	...
	notify_property_list_changed()
	_changed()
func _set_dash_fill_count(value: int) -> void:
	var next := clampi(value, 0, dash_total_count)
	if dash_fill_count == next: return
	dash_fill_count = next
	_changed()
func _set_dash_total_count(value: int) -> void:
	var next := clampi(value, 1, 4096)
	var next_fill := mini(dash_fill_count, next)
	if dash_total_count == next and dash_fill_count == next_fill: return
	dash_total_count = next
	dash_fill_count = next_fill
	_changed()
...
func _validate_property(property: Dictionary) -> void:
	...
	var visible := true
	if property_name == "size_2d": visible = not line and kind in [MoldShape.Kind.RECTANGLE, MoldShape.Kind.RECTANGLE_RIM]
	elif property_name == "size_3d": visible = not line and kind == MoldShape.Kind.CUBOID
	elif property_name == "radius": visible = not line and kind not in [MoldShape.Kind.RECTANGLE, MoldShape.Kind.RECTANGLE_RIM, MoldShape.Kind.CUBOID]
	elif property_name == "height": visible = not line and kind in [MoldShape.Kind.CYLINDER, MoldShape.Kind.REGULAR_PRISM, MoldShape.Kind.CONE, MoldShape.Kind.CAPSULE]
	elif property_name == "thickness": visible = line or kind in [MoldShape.Kind.RECTANGLE_RIM, MoldShape.Kind.RING, MoldShape.Kind.REGULAR_POLYGON_RIM, MoldShape.Kind.TORUS]
	...
	elif property_name in ["start_degrees", "span_degrees"]: visible = kind in [MoldShape.Kind.RECTANGLE, MoldShape.Kind.RECTANGLE_RIM, MoldShape.Kind.DISC, MoldShape.Kind.RING, MoldShape.Kind.REGULAR_POLYGON, MoldShape.Kind.REGULAR_POLYGON_RIM, MoldShape.Kind.TORUS]
	elif property_name == "sides": visible = kind in [MoldShape.Kind.REGULAR_POLYGON, MoldShape.Kind.REGULAR_POLYGON_RIM, MoldShape.Kind.REGULAR_PRISM]
	...
	elif property_name in ["dash_fill_count", "dash_total_count", "dash_spacing", "dash_offset"]: visible = (_is_rim(kind) or line) and dashed
...
static func _is_2d_kind(value: MoldShape.Kind) -> bool:
	return value in [MoldShape.Kind.RECTANGLE, MoldShape.Kind.RECTANGLE_RIM, MoldShape.Kind.DISC, MoldShape.Kind.RING, MoldShape.Kind.REGULAR_POLYGON, MoldShape.Kind.REGULAR_POLYGON_RIM]


static func _is_rim(value: MoldShape.Kind) -> bool:
	return value in [MoldShape.Kind.RECTANGLE_RIM, MoldShape.Kind.RING, MoldShape.Kind.REGULAR_POLYGON_RIM]
...
func to_shape() -> MoldShape:
	...
	if dashed:
		var total := maxi(1, dash_total_count)
		dash = MoldDash.new(clampi(dash_fill_count, 0, total), total, dash_spacing, dash_offset, dash_type, dash_modifier)
	...
	match kind:
		...
		MoldShape.Kind.RING: shape = MoldShape.ring(safe_radius, safe_thickness, deg_to_rad(start_degrees), deg_to_rad(clampf(span_degrees, 0.0, 360.0)), dash)

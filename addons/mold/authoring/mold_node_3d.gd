...
func _notification(what: int) -> void:
	if what == NOTIFICATION_TRANSFORM_CHANGED and _handle and _handle.is_valid:
...
func _connect_resources() -> void:
	...
	# Remove connections created by versions that routed every resource change
	# through the full geometry refresh.
	if shape and shape.changed.is_connected(_refresh): shape.changed.disconnect(_refresh)
	if style and style.changed.is_connected(_refresh): style.changed.disconnect(_refresh)
...
func _reset_invalid_color_mode() -> void:
	...
	var valid := style.color_mode == MoldStyle.ColorMode.SINGLE or style.color_mode == MoldStyle.ColorMode.DUAL_GRADIENT or (style.color_mode in [MoldStyle.ColorMode.DUAL_RADIAL, MoldStyle.ColorMode.DUAL_RADIAL_BOUNDS, MoldStyle.ColorMode.DUAL_ANGULAR] and not value_shape.is_line)
...
func _refresh_preview() -> void:
	...
	_preview.mesh = _preview_cache.mesh_for_testing(value_shape)
...
func _is_preview_child(candidate: MeshInstance3D) -> bool:
	if candidate.has_meta(PREVIEW_MARKER) or String(candidate.name).begins_with(PREVIEW_NODE_NAME): return true
	# Pre-fix preview nodes can lack the marker after a tool reload. Only adopt
	# unowned internal nodes; regular user-authored MeshInstance3D children stay intact.
...
func _refresh_preview_material() -> void:
	...
	if style.mode == MoldStyle.BlendMode.CUSTOM:
		...
		var custom_deformation := 1.0 if value_shape.kind == MoldShape.Kind.CAPSULE else (2.0 if value_shape.kind == MoldShape.Kind.TORUS else 0.0)
		var custom_flags := 1 \
		...
		custom.set_shader_parameter("mold_instance_data", MoldMultiMeshRenderer.create_single_instance_texture(
			style.color, layout.custom_data, value_shape.dash.to_gpu_data(value_shape.dash_path_length()),
	...
	var shader := MoldShader.preview_shader(value_style.mode, state, value_shape.is_2d or value_shape.is_line)
	...
	var deformation := 1.0 if value_shape.kind == MoldShape.Kind.CAPSULE else (2.0 if value_shape.kind == MoldShape.Kind.TORUS else 0.0)
	var packed_flags := 33 \
		+ 2 * int(value_shape.billboard_mode != MoldShape.BillboardMode.DISABLED) \
		+ 4 * int(value_shape.is_line) \
		+ 16 * int(value_shape.billboard_mode == MoldShape.BillboardMode.FACE_CAMERA_Y)
	...
	if value_style.color_mode == MoldStyle.ColorMode.DUAL_RADIAL and value_shape.kind in [MoldShape.Kind.RECTANGLE, MoldShape.Kind.RECTANGLE_RIM, MoldShape.Kind.REGULAR_POLYGON, MoldShape.Kind.REGULAR_POLYGON_RIM]:
...
func _angular_lod_data(value_shape: MoldShape) -> Vector4:
...
func get_mold_aabb() -> AABB:
	...
	var mesh := _preview_cache.mesh_for_testing(value_shape)
